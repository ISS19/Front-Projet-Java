  ...
  const Login = async (e) => {
    ...
    UserService.login(user)
      .then((res) => {
        ...
        if (res.status === 200 && res.data.status == "Admin") {
          router.push('/Admin/statistique');
          localStorage.setItem("user-session", JSON.stringify(res.data.nom));
          localStorage.setItem("user-session-id", JSON.stringify(res.data.id));
        }
        else if (res.status === 200 && res.data.status == "Chef de secteur") {
